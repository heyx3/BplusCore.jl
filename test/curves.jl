tests = [
    ...
    (
        ...
        t -> lerp(v3f(1, 2, 3), v3f(4, 6, 8),
            let t_clamped = clamp(t, 0.0f0, 1.0f0),
                t_inv = 1.0f0 - t_clamped,
                t2 = t_clamped * t_clamped,
                t2_inv = t_inv * t_inv
                t_bez = (t2_inv * t_inv * 0.2f0) +
                         (3.0f0 * t2_inv * t_clamped * (0.2f0 + 2.0f0)) +
    ...
    (
        ...
        _t -> let t = lerp(-0.2f0, 1.0f0, clamp(_t, 0.0f0, 1.0f0))
            if t < 0.2
...
for (curve, expected_fn, t_min, t_max, name) in tests
    ...
    for t in (t_min-t_inflation) : t_step : (t_max+t_inflation)
        ...
        @noinline get_test_data() = let actual = curve_eval(curve, t),
                                        expected = expected_fn(inv_lerp(t_min, t_max, t))
            (actual, expected, get_test_error(actual, expected))
        end

        @bp_test_no_allocations(get_test_data()[3] < 0.00001f0,
                                true,
                                name, ": ", curve, " at t=", t,
